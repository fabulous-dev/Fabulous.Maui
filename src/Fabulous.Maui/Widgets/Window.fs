...
module Window =
    ...
    let Content = Attributes.defineMauiPropertyWidget "Window_Content" "Content" (fun target -> (target :?> FabWindow).Content) (fun target value -> (target :?> FabWindow).Content <- value)
