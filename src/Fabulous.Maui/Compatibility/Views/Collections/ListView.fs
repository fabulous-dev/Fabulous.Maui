...
type IFabCompatListView =
    inherit IFabCompatItemsViewOfCell

module ListView =

    let WidgetKey = CompatWidgets.register<FabulousListView>()
...
module ListViewBuilders =
    type Fabulous.Maui.View with
        ...
        static member inline ListView<'msg, 'itemData, 'itemMarker when 'itemMarker :> IFabCompatCell>(items: seq<'itemData>) =
            WidgetHelpers.buildItems<'msg, IFabCompatListView, 'itemData, 'itemMarker> ListView.WidgetKey ItemsViewOfCell.ItemsSource items
        ...
        static member inline GroupedListView<'msg, 'groupData, 'groupMarker, 'itemData, 'itemMarker
            ...
            (items: seq<'groupData>)
            =
            WidgetHelpers.buildGroupItemsNoFooter<'msg, IFabCompatListView, 'groupData, 'itemData, 'groupMarker, 'itemMarker>
                ListView.WidgetKey
...
type ListViewModifiers =
    ...
    [<Extension>]
    static member inline hasUnevenRows(this: WidgetBuilder<'msg, #IFabCompatListView>, value: bool) =
        this.AddScalar(ListView.HasUnevenRows.WithValue(value))

    [<Extension>]
    static member inline horizontalScrollBarVisibility(this: WidgetBuilder<'msg, #IFabCompatListView>, value: ScrollBarVisibility) =
        this.AddScalar(ListView.HorizontalScrollBarVisibility.WithValue(value))

    [<Extension>]
    static member inline verticalScrollBarVisibility(this: WidgetBuilder<'msg, #IFabCompatListView>, value: ScrollBarVisibility) =
        this.AddScalar(ListView.VerticalScrollBarVisibility.WithValue(value))

    [<Extension>]
    static member inline isPullToRefreshEnabled(this: WidgetBuilder<'msg, #IFabCompatListView>, value: bool) =
        this.AddScalar(ListView.IsPullToRefreshEnabled.WithValue(value))

    [<Extension>]
    static member inline isRefreshing(this: WidgetBuilder<'msg, #IFabCompatListView>, value: bool) =
        this.AddScalar(ListView.IsRefreshing.WithValue(value))

    [<Extension>]
    static member inline refreshControlColor(this: WidgetBuilder<'msg, #IFabCompatListView>, light: FabColor, ?dark: FabColor) =
        this.AddScalar(ListView.RefreshControlColor.WithValue(AppTheme.create light dark))

    [<Extension>]
    static member inline separatorColor(this: WidgetBuilder<'msg, #IFabCompatListView>, light: FabColor, ?dark: FabColor) =
        this.AddScalar(ListView.SeparatorColor.WithValue(AppTheme.create light dark))

    [<Extension>]
    static member inline separatorVisibility(this: WidgetBuilder<'msg, #IFabCompatListView>, value: SeparatorVisibility) =
        this.AddScalar(ListView.SeparatorVisibility.WithValue(value))

    [<Extension>]
    static member inline rowHeight(this: WidgetBuilder<'msg, #IFabCompatListView>, value: int) =
        this.AddScalar(ListView.RowHeight.WithValue(value))

    [<Extension>]
    static member inline selectionMode(this: WidgetBuilder<'msg, #IFabCompatListView>, value: ListViewSelectionMode) =
        this.AddScalar(ListView.SelectionMode.WithValue(value))

    [<Extension>]
    static member inline onItemAppearing(this: WidgetBuilder<'msg, #IFabCompatListView>, onItemAppearing: ItemVisibilityEventArgs -> 'msg) =
        this.AddScalar(ListView.ItemAppearing.WithValue(fun args -> onItemAppearing args |> box))

    [<Extension>]
    static member inline onItemDisappearing(this: WidgetBuilder<'msg, #IFabCompatListView>, onItemDisappearing: ItemVisibilityEventArgs -> 'msg) =
        this.AddScalar(ListView.ItemDisappearing.WithValue(fun args -> onItemDisappearing args |> box))

    [<Extension>]
    static member inline onItemTapped(this: WidgetBuilder<'msg, #IFabCompatListView>, onItemTapped: int -> 'msg) =
    ...
    static member inline onItemSelected(this: WidgetBuilder<'msg, #IFabCompatListView>, onItemSelected: SelectedItemChangedEventArgs -> 'msg) =
        this.AddScalar(ListView.ItemSelected.WithValue(fun args -> onItemSelected args |> box))

    [<Extension>]
    static member inline onItemSelected(this: WidgetBuilder<'msg, #IFabCompatListView>, onItemSelected: int -> 'msg) =
        this.AddScalar(ListView.ItemSelected.WithValue(fun args -> onItemSelected args.SelectedItemIndex |> box))

    [<Extension>]
    static member inline onRefreshing(this: WidgetBuilder<'msg, #IFabCompatListView>, onRefreshing: 'msg) =
        this.AddScalar(ListView.Refreshing.WithValue(onRefreshing))

    [<Extension>]
    static member inline onScrolled(this: WidgetBuilder<'msg, #IFabCompatListView>, onScrolled: ScrolledEventArgs -> 'msg) =
        this.AddScalar(ListView.Scrolled.WithValue(fun args -> onScrolled args |> box))

    [<Extension>]
    static member inline onScrollToRequested(this: WidgetBuilder<'msg, #IFabCompatListView>, onScrollToRequested: ScrollToRequestedEventArgs -> 'msg) =
    ...
    [<Extension>]
    static member inline reference(this: WidgetBuilder<'msg, IFabCompatListView>, value: ViewRef<ListView>) =
